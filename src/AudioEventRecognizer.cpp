...
namespace vibhearing {
	namespace {
		...
		float calculateAcousticStrength(const AudioFeatures& features) {
			...
			const float decibels = kDecibelsPerAmplitudeRatio * std::log10(std::max(signalToNoise, 1.0F));
			return std::clamp((decibels - kMinimumStrengthDb) / (kMaximumStrengthDb - kMinimumStrengthDb), 0.0F, 1.0F);
		}
	...
	AudioEvent AudioEventRecognizer::recognize(const AudioFeatures& features, const uint32_t timestampMs) {
