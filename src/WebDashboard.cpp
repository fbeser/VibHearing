...
</style></head><body><header><div><h1>VibHearing</h1><div class="muted">Akustik telemetri ve haptik kontrol</div></div><div class="badge" id="connection">Bağlanıyor…</div></header><main>
<section class="card metrics"><div class="metric"><small>Olay</small><strong id="event">—</strong></div><div class="metric"><small>Güven</small><strong id="confidence">0%</strong></div><div class="metric"><small>SNR</small><strong id="snr">0.00</strong></div><div class="metric"><small>RMS</small><strong id="rms">0</strong></div><div class="metric"><small>Peak</small><strong id="peak">0</strong></div><div class="metric"><small>Haptik güç</small><strong id="strength">0%</strong></div></section>
<section class="card wave"><h2>Ham ses dalga biçimi</h2><canvas id="waveform" width="900" height="230"></canvas><p class="muted">Son I2S karesinin 256 normalize örneği; görüntü otomatik ölçeklenir.</p></section>
<section class="card bands"><h2>FFT frekans bantları</h2><div id="bandList"></div></section>
<section class="card settings"><h2>Canlı ayarlar</h2><form id="settingsForm">
<label class="check"><input type="checkbox" id="motorEnabled"><span>Motor etkin</span></label><label class="check"><input type="checkbox" id="serialMetricsEnabled"><span>Seri telemetri</span></label>
<label><span>Minimum voice SNR</span><input id="minimumVoiceSnr" type="number" min="1" max="10" step="0.05"></label><label><span>Minimum voice oranı</span><input id="minimumVoiceRatio" type="number" min="0.1" max="0.9" step="0.01"></label>
<label><span>Yakın ses SNR</span><input id="closeVoiceSnr" type="number" min="2" max="30" step="0.1"></label><label><span>Minimum güven</span><input id="minimumEventConfidence" type="number" min="0.5" max="0.99" step="0.01"></label>
<label><span>Minimum motor PWM</span><input id="minimumHapticDuty" type="number" min="0" max="180" step="1"></label><label><span>Maksimum motor PWM</span><input id="maximumHapticDuty" type="number" min="0" max="180" step="1"></label>
<label><span>Tekrar koruması (ms)</span><input id="eventRetriggerGuardMs" type="number" min="200" max="5000" step="10"></label><button type="submit">Ayarları uygula</button><div class="full" id="message"></div>
</form></section>
<section class="card device"><h2>Cihaz</h2><dl><dt>IP</dt><dd id="ip">—</dd><dt>Wi-Fi RSSI</dt><dd id="rssi">—</dd><dt>Çalışma</dt><dd id="uptime">—</dd><dt>Örnekleme</dt><dd>16 kHz</dd><dt>FFT</dt><dd>256 örnek</dd><dt>Motor pini</dt><dd>GPIO7</dd><dt>I2S</dt><dd>4 / 5 / 6</dd></dl></section>
</main><script>
...
async function poll(){try{const r=await fetch('/api/status',{cache:'no-store'}),d=await r.json();set('connection',d.wifi?'Çevrimiçi':'Wi-Fi bekleniyor');set('event',d.event);set('confidence',Math.round(d.confidence*100)+'%');set('strength',Math.round(d.strength*100)+'%');set('snr',d.snr.toFixed(2));set('rms',d.rms.toFixed(5));set('peak',d.peak.toFixed(5));set('ip',d.ip);set('rssi',d.rssi+' dBm');set('uptime',Math.floor(d.uptime/60)+' dk');d.bands.forEach((v,i)=>{set('bv'+i,v.toFixed(3));document.getElementById('b'+i).style.width=Math.min(100,Math.log10(1+v*18)*58)+'%'});drawWave(d.waveform)}catch(e){set('connection','Bağlantı yok')}setTimeout(poll,250)}
