...
namespace vibhearing {
	...
	HumanVoiceDetector::HumanVoiceDetector(const RuntimeSettings& settings) : settings_(settings) {
	}
	...
	float HumanVoiceDetector::detect(const AudioFeatures& features) {
		const float bassEnergy = features.bands[static_cast<size_t>(FrequencyBand::Bass)];
		const float highEnergy = features.bands[static_cast<size_t>(FrequencyBand::High)];
		const float voiceEnergy = features.bands[static_cast<size_t>(FrequencyBand::LowMid)] +
		...
		const float totalEnergy = bassEnergy + voiceEnergy + highEnergy * (1.0F - kHighBandVoiceWeight);
		const float voiceRatio = voiceEnergy / std::max(totalEnergy, 0.000001F);
		const float signalToNoise = features.rms / std::max(features.noiseFloor, 0.000001F);
		float instantVariation = 0.0F;
		if (hasPreviousFrame_) {
			const float rmsChange =
				std::abs(features.rms - previousRms_) / std::max({features.rms, previousRms_, 0.000001F});
			...
			const float spectralChange = bandChange / std::max(bandMagnitude, 0.000001F);
			instantVariation = std::clamp(0.65F * rmsChange + 0.35F * spectralChange, 0.0F, 1.0F);
		}
		previousRms_ = features.rms;
		previousBands_ = features.bands;
		hasPreviousFrame_ = true;
		temporalVariation_ += (instantVariation - temporalVariation_) * kVariationSmoothing;
		...
		const bool closeOrClippedVoice = signalToNoise >= settings_.closeVoiceSnr &&
										 ...
										 temporalVariation_ >= kCloseVoiceMinimumVariation;
		...
		const bool active = activeFrames_ >= kAttackFrames && inactiveFrames_ < kReleaseFrames;
		if (!active) {
			return 0.0F;
		}
		const float ratioScore =
			std::clamp((voiceRatio - settings_.minimumVoiceRatio) / (1.0F - settings_.minimumVoiceRatio), 0.0F, 1.0F);
		const float noiseScore = std::clamp((signalToNoise - settings_.minimumVoiceSnr) / 3.0F, 0.0F, 1.0F);
