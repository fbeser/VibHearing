...
namespace {
	...
	MotorDriver rightMotor(static_cast<uint8_t>(config::kMotorRightPin), config::kEnableRightMotor);
	...
	void logMetrics(const AudioFeatures& features, const AudioEvent& event, const uint32_t nowMs) {
		if (!runtimeSettings.serialMetricsEnabled || nowMs - lastMetricsLogMs < config::kMetricsLogPeriodMs) {
			return;
		}
		...
		Serial.printf(
			...
			features.rms, features.peak, features.gain, features.noiseFloor, signalToNoise, audioEventName(event.type),
			event.confidence, event.acousticStrength, features.bands[0], features.bands[1], features.bands[2],
