...
namespace vibhearing {
	...
	bool AudioEngine::begin() {
		...
		esp_err_t result = i2s_new_channel(&channelConfig, nullptr, &receiveChannel_);
		if (result != ESP_OK) {
	...
	bool AudioEngine::capture(AudioFeatures& features) {
		...
		const esp_err_t result =
			i2s_channel_read(receiveChannel_, rawSamples_.data(), sizeof(rawSamples_), &bytesRead, 100);
		if (result != ESP_OK || bytesRead == 0U) {
			Logger::error("I2S read failed", result);
			return false;
		}

		buffer_.load(rawSamples_.data(), bytesRead / sizeof(rawSamples_[0]), config::kI2sSlotsPerFrame);
	...
	void AudioEngine::extractLevelFeatures(AudioFeatures& features) {
		...
		constexpr size_t kWaveformStride = config::kAudioFrameSamples / config::kWaveformSamples;
		static_assert(kWaveformStride > 0U, "Waveform sample count cannot exceed the audio frame");
		for (size_t index = 0; index < features.waveform.size(); ++index) {
			const float sample = std::clamp(buffer_.samples()[index * kWaveformStride], -1.0F, 1.0F);
			features.waveform[index] = static_cast<int16_t>(sample * static_cast<float>(INT16_MAX));
		}
	}

	void AudioEngine::updateAdaptiveLevels(AudioFeatures& features) {
		const float desiredGain =
			std::clamp(kTargetRms / std::max(features.rms, kMinimumNoiseFloor), kMinimumGain, kMaximumGain);
		gain_ += (desiredGain - gain_) * kGainSmoothing;

		const float noiseRate = features.rms < noiseFloor_ ? kNoiseFallRate : kNoiseRiseRate;
