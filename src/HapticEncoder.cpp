...
namespace vibhearing {
	...
	HapticEncoder::HapticEncoder(MotorDriver& leftMotor, MotorDriver& rightMotor, const RuntimeSettings& settings)
	...
	void HapticEncoder::handleEvent(const AudioEvent& event) {
		if (!settings_.motorEnabled || event.type != AudioEventType::HumanVoice ||
			...
			(lastEventMs_ != 0U && event.timestampMs - lastEventMs_ < settings_.eventRetriggerGuardMs)) {
			return;
		}
	...
	bool HapticEncoder::isActive() const {
		return active_;
	}

	void HapticEncoder::startPattern(const AudioEventType type, const uint32_t nowMs, const float acousticStrength) {
	...
	uint8_t HapticEncoder::intensityForStrength(const float acousticStrength) const {
		const float boundedStrength = std::clamp(acousticStrength, 0.0F, 1.0F);
		const float dutyRange = static_cast<float>(settings_.maximumHapticDuty - settings_.minimumHapticDuty);
		return static_cast<uint8_t>(std::lround(settings_.minimumHapticDuty + boundedStrength * dutyRange));
	}
