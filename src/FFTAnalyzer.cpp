...
namespace vibhearing {
	namespace {
		...
		constexpr std::array<float, 7> kBandEdgesHz{60.0F, 250.0F, 500.0F, 1000.0F, 2000.0F, 4000.0F, 8000.0F};
		constexpr size_t kFftBits = 8;
		static_assert((1U << kFftBits) == config::kAudioFrameSamples, "FFT frame size must be a power of two");

	}  // namespace

	BandEnergies FFTAnalyzer::analyze(const AudioBuffer::Samples& samples, const float gain) {
		for (size_t index = 0; index < samples.size(); ++index) {
			const float window = 0.5F - 0.5F * std::cos((2.0F * kPi * index) / (samples.size() - 1U));
		...
		for (size_t bin = 1; bin <= frame_.size() / 2U; ++bin) {
			const float frequency = static_cast<float>(bin * config::kSampleRateHz) / frame_.size();
			for (size_t band = 0; band < energies.size(); ++band) {
				const bool isLastBand = band == energies.size() - 1U;
				if (frequency >= kBandEdgesHz[band] &&
					(frequency < kBandEdgesHz[band + 1U] || (isLastBand && frequency <= kBandEdgesHz[band + 1U]))) {
		...
		for (size_t band = 0; band < energies.size(); ++band) {
			if (binCounts[band] > 0U) {
				energies[band] = std::sqrt(energies[band] / static_cast<float>(binCounts[band]));
			}
		}
		return energies;
	}
