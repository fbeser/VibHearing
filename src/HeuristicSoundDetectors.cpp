...
namespace vibhearing {
	namespace {
		...
		float normalizedScore(const float value, const float minimum, const float maximum) {
	...
	float AnimalVocalizationDetector::detect(const AudioFeatures& features) {
		...
		const float vocalMagnitude = sound_features::bandMagnitude(features, FrequencyBand::Mid) +
		...
		const bool candidate = snr >= 1.9F && vocalRatio >= 0.20F && centroid >= 650.0F && centroid <= 3200.0F;
		activeFrames_ = candidate ? std::min<uint8_t>(activeFrames_ + 1U, 4U) : 0U;
		if (activeFrames_ < 4U) {
			return 0.0F;
		}
		return 0.82F + 0.10F * normalizedScore(vocalRatio, 0.20F, 0.60F) + 0.06F * normalizedScore(snr, 1.9F, 8.0F);
	}
	...
	float TonalAlertDetector::detect(const AudioFeatures& features) {
		const float snr = sound_features::signalToNoise(features);
		const float usableMagnitude = sound_features::bandMagnitude(features, FrequencyBand::Mid) +
									  sound_features::bandMagnitude(features, FrequencyBand::Voice) +
									  sound_features::bandMagnitude(features, FrequencyBand::Presence) +
									  sound_features::bandMagnitude(features, FrequencyBand::High);
		const float dominantMagnitude = std::max({sound_features::bandMagnitude(features, FrequencyBand::Mid),
												  sound_features::bandMagnitude(features, FrequencyBand::Voice),
												  sound_features::bandMagnitude(features, FrequencyBand::Presence),
												  sound_features::bandMagnitude(features, FrequencyBand::High)});
		const float concentration = dominantMagnitude / std::max(usableMagnitude, 0.000001F);
		const bool candidate = snr >= 2.0F && concentration >= 0.66F &&
							   usableMagnitude >= sound_features::bandMagnitude(features, FrequencyBand::Bass) * 0.25F;
	...
	float ContinuousNoiseDetector::detect(const AudioFeatures& features) {
		const float snr = sound_features::signalToNoise(features);
		const float relativeChange = std::abs(features.rms - previousRms_) / std::max(previousRms_, 0.000001F);
