...
void assertDominantBand(const float frequencyHz,
                        const vibhearing::FrequencyBand expectedBand) {
...
void test_feature_math_handles_zero_floor_and_empty_spectrum() {
  ...
  TEST_ASSERT_FLOAT_WITHIN(
      1.0F, 500000.0F, vibhearing::sound_features::signalToNoise(features));
  TEST_ASSERT_FLOAT_WITHIN(
      0.000001F, 0.0F,
      vibhearing::sound_features::spectralCentroidHz(features));
  TEST_ASSERT_FLOAT_WITHIN(
      0.000001F, 1.5F, vibhearing::sound_features::crestFactor(features));
}
